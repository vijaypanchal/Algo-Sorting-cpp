...
#define print_clock(VAR,CLOCK_TYPE)             \
	if(diff_##VAR##CLOCK_TYPE.tv_nsec < 0) diff_##VAR##CLOCK_TYPE.tv_nsec*=-1;\
	std::cout << "Started Time for " << #VAR << " is sec " << start_##VAR##CLOCK_TYPE.tv_sec << " and nsec " << start_##VAR##CLOCK_TYPE.tv_nsec << std::endl;\
	std::cout << "Stopped Time for " << #VAR << " is sec " << stop_##VAR##CLOCK_TYPE.tv_sec << " and nsec " << stop_##VAR##CLOCK_TYPE.tv_nsec << std::endl;\
	std::cout << "Total Time By " << #VAR << " is sec " << diff_##VAR##CLOCK_TYPE.tv_sec << " and nsec " << diff_##VAR##CLOCK_TYPE.tv_nsec << std::endl;

#else
#define print_clock(VAR,CLOCK_TYPE)             \
	if(diff_##VAR##CLOCK_TYPE.tv_nsec < 0) diff_##VAR##CLOCK_TYPE.tv_nsec*=-1;\
	printf("Start Time for %15s is sec %15ld and nsec %15ld \n",#VAR, start_##VAR##CLOCK_TYPE.tv_sec, start_##VAR##CLOCK_TYPE.tv_nsec); \
	printf("Stop Time for %15s is sec %15ld and nsec %15ld \n",#VAR, stop_##VAR##CLOCK_TYPE.tv_sec, stop_##VAR##CLOCK_TYPE.tv_nsec); \
	printf("Total Time for %15s is sec %15ld and nsec %15ld \n",#VAR, diff_##VAR##CLOCK_TYPE.tv_sec, diff_##VAR##CLOCK_TYPE.tv_nsec);
#endif
